...
const csvRows = parseCSVToRows(csvText);
...
for (const row of csvRows) {
  ...
  const sheetStatus = determineStatusFromSheet(row);
  ...
  if (MANUAL_SHEET_MAP[sheetTeamName]) {
    ...
    if (order) {
      ...
      const dateChanges = compareDates(order.data, sheetDates);
