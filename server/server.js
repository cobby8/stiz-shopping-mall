...
import knowledgeAdminRoutes from './routes/knowledge.js';     // 챗봇 지식베이스 관리자 API (K3 Phase 3A)
import { adminAuth } from './middleware/adminAuth.js';
// API 레이트 리밋 미들웨어 (P0-2, R-02) — 봇/공격자의 비용 폭탄·brute force 차단
...
// ============================================================
// API 레이트 리밋 부착 (P0-2, R-02) — 라우트 마운트 직전에 위치
// ============================================================
// 비유: 식당 입구에 줄 정리 직원을 세우는 것. 손님이 너무 많이 시키면
//       "잠시만요"라고 말려준다. 봇이 1초에 1만 번 호출해서 비용을 폭주시키거나
//       PG사에 차단당하는 사고를 막는다.
// 관리자 토큰(role=admin, 서명 검증 통과)은 skip → 운영자가 자기 발에 안 걸림.
app.use('/api/auth/login', loginLimiter);  // 1분 5회 — brute force 차단
app.use('/api/generate', aiLimiter);        // 1분 10회 — Gemini 비용 폭탄 차단 (chat 포함)
app.use('/api/payment', paymentLimiter);    // 1분 20회 — PG사 차단 위험 회피
...
// 챗봇 지식베이스 관리자 라우트 (K3 Phase 3A)
// ⚠️ /api/admin 보다 먼저 마운트해야 /api/admin/knowledge 가 우선 매칭됨 (Express 순서 민감)
// adminAuth 미들웨어 통과한 요청만 knowledge CRUD/재빌드 엔드포인트 접근 가능
app.use('/api/admin/knowledge', adminAuth, knowledgeAdminRoutes);
