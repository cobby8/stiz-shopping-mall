...
function buildHeaderToolbar() {
    ...
    // C-9/D-95: 공통 헬퍼 경유 (admin-common.js isAdminMobile / isAdminTabletOnly)
    const isMobile = isAdminMobile();
    const isTablet = isAdminTabletOnly();
    // 모바일(<768px): 뷰 토글 숨김(initialView=dayGridMonth로 충분, 좁은 헤더 줄바꿈 방지)
    // 태블릿(768~1279px): 월간만 / PC(≥1280px): 월간+주간
    const right = isMobile ? '' : (isTablet ? 'dayGridMonth' : 'dayGridMonth,timeGridWeek');
    return {
        left: 'prev,next today',                                       // 좌측: 이전/다음/오늘 버튼
        center: 'title',                                               // 중앙: "2026년 4월" 같은 제목
        right                                                          // 모바일: 빈 문자열 / 태블릿/PC: 위 분기 결과
    };
}
